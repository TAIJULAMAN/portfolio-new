    ...
    return (
        <>
            ...
            <header className="border-b border-gray-700 bg-gray-900/90 backdrop-blur-sm sticky top-0 z-50">
                ...
                <div 
                    className={`fixed top-0 left-0 h-screen w-64 bg-gray-900 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out ${
                    ...
                    <div className="h-full flex flex-col pt-24 px-6 space-y-4 overflow-y-auto">
                        {navLinks.map((link, index) => (
                            <Link 
                                key={index}
                                to={link.to}
                                className={`px-4 py-3 text-lg rounded-md transition-all duration-200 transform hover:translate-x-1 ${
                                    isActive(link.to)
